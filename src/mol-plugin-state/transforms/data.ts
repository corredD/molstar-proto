...
import { parseObj } from '../../mol-io/reader/obj/parser';
...
const ParseObj = PluginStateTransform.BuiltIn({
    name: 'parse-obj',
    display: { name: 'Parse OBJ', description: 'Parse Wavefront OBJ from String data' },
    from: [SO.Data.String],
    to: SO.Format.Obj
})({
    apply({ a }) {
        return Task.create('Parse OBJ', async ctx => {
            const parsed = await parseObj(a.data).runInContext(ctx);
            if (parsed.isError) throw new Error(parsed.message);
            return new SO.Format.Obj(parsed.result, { label: parsed.result.comments[0] || 'OBJ Data' });
        });
    }
});
