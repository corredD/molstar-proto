...
/*
const PostprocessingSchema:RenderableSchema = {
    ...QuadSchema,
    tColor: TextureSpec('texture', 'rgba', 'ubyte', 'nearest'),
    tDepth: TextureSpec('texture', 'rgba', 'ubyte', 'nearest'),
    uTexSize: UniformSpec('v2'),
    dSSAOEnable: DefineSpec('boolean'),
    dssao_samples: DefineSpec('number'),
    dssao_rings: DefineSpec('number'),
    ussao_scale: UniformSpec('f'),
    ussao_aoCap: UniformSpec('f'),
    ussao_aoMultiplier: UniformSpec('f'),
    ussao_depthTolerance: UniformSpec('f'),
    ussao_aorange: UniformSpec('f'),

    dHashEnable: DefineSpec('boolean'),
...
export const PostprocessingParams = {
    ssaoEnable: PD.Boolean(false),
    ssao_samples: PD.Numeric(4, { min: 1, max: 100, step: 1 }),
    ssao_rings: PD.Numeric(4, { min: 1, max: 100, step: 1 }),
    ssao_scale: PD.Numeric(1, { min: 1, max: 100, step: 1 }),
    ssao_aoCap: PD.Numeric(1.0, { min: 0, max: 10, step: 0.1 }),
    ssao_aoMultiplier: PD.Numeric(100, { min:1, max: 500, step: 1 }),
    ssao_depthTolerance: PD.Numeric(0, { min: 0, max: 1, step: 0.01 }),
    ssao_aorange: PD.Numeric(160.0, { min: 1, max: 500, step: 1 }),

    hashEnable: PD.Boolean(false),
...
export function getPostprocessingRenderable(ctx: WebGLContext,
  colorTexture: Texture, depthTexture: Texture, props: Partial<PostprocessingProps>) {
    //RenderableValues
    ...
    const values: Values<typeof PostprocessingSchema> = {
        ...
        /*
        dSSAOEnable: ValueCell.create(p.ssaoEnable),
        dssao_samples: ValueCell.create(p.ssao_samples),
        dssao_rings: ValueCell.create(p.ssao_rings),
        ussao_scale: ValueCell.create(p.ssao_scale),
        ussao_aoCap: ValueCell.create(p.ssao_aoCap),
        ussao_aoMultiplier: ValueCell.create(p.ssao_aoMultiplier),
        ussao_depthTolerance: ValueCell.create(p.ssao_depthTolerance),
        ussao_aorange: ValueCell.create(p.ssao_aorange),

        dHashEnable: ValueCell.create(p.hashEnable),
